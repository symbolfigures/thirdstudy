...
class Checkpointer:
	...
	def last_checkpoint(self):
		...
		checkpoint_nos = [int(f.split('.')[0]) for f in files if f.endswith('.checkpoint')]
		return f'{max(checkpoint_nos)}'
	...
	def load_checkpoint(self) -> bytes:
		return read_binary_file(self.path_for_checkpoint(self.last_checkpoint()))
